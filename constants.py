...
class Constants:
    ...
    try:
        ...
        display_str = f"Seçtiğiniz brawler {brawler_name.upper()} değilse {brawler_name.upper()} istatistiklerini kullanarak,\nlütfen constants.py dosyasından manuel olarak değiştirin."
        standard_hsf = 0.15
        if len(brawler_stats) == 2:
            brawler_stats.append(standard_hsf)
        elif len(brawler_stats) > 3:
            display_str = f"brawler_stats.json dosyasındaki {brawler_name} öğesinin 3'ten fazla öğesi var, constants.py dosyasında istatistikler kullanılıyor"
            brawler_stats = 3*[None]
    except KeyError:
        brawler_stats = 3*[None]
        display_str = f"{brawler_name.upper()} istatistikleri JSON'da bulunamadı. \constant.py de hız, saldırı_menzili ve yükseklik faktörü kullanılıyor.\nEğer yapmadıysanız lütfen constants.py de manuel olarak değiştirin."
    print("")
    print(bcolors.BOLD + bcolors.OKGREEN + "Original Creator: https://github.com/Jooi025/BrawlStarsBot" + bcolors.ENDC)
    print("")
    print(bcolors.BOLD + bcolors.OKGREEN + "Güncelleyen ve Düzelten: https://github.com/MembaCo/BrawlStarsBot-Mc" + bcolors.ENDC)
    print("")
    ...
    print("")
    print(bcolors.OKBLUE + f"Hız: {speed} adım/saniye \nSaldırı Mesafesi: {attack_range} adım\nYükseklik Ölçekleme Faktörü: {heightScaleFactor}" + bcolors.ENDC)
