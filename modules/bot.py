...
class Brawlbot:
    ...
    def move_to_bush(self):
        ...
        if self.bushResult:
            ...
            print(f"Mesafe: {round(tileDistance,2)} adım")
            return moveTime
        ...
        if self.results:
            ...
            if self.results[self.enemy_index]:
                ...
                if self.enemyResults:
                    ...
                    print(f"En yakın düşman: {round(enemyDistance,2)} adım uzakta")
            ...
            if self.results[self.player_index]:
                ...
                if self.last_player_pos is None:
                    ...
                    if self.last_player_pos == player_pos:
                        self.counter += 1
                        if self.counter == 2:
                            print("Hareket etmeyi bıraktınız veya Sıkıştınız")
                            return True
                    else:
                        # reset counter
                        self.counter = 0
        ...
        while not self.stopped:
            ...
            if self.state == BotState.INITIALIZING:
                ...
                if time() > self.timestamp + self.HIDINGTIME or self.is_player_damaged():
                    print("Değiştirilecek Konum Aranıyor")
                    self.lock.acquire()
                    self.state = BotState.SEARCHING
                    self.lock.release()
